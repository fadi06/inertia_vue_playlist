        ...
        <div v-if="Object.keys(listing.data).length">
            <div class="mb-6">
                ...
                <table class="w-full table-fixed border-collapse overflow-hidden rounded-md text-sm ring-1 ring-slate-300 dark:ring-slate-600 bg-white shadow-lg">
                    ...
                    <tbody>
                        <tr
                            v-for="listing in listing.data" :key="listing.id"
                            ...
                            <td class="w-3/4 p-3 text-left">
                                <div class="flex items-center gap-2">
                                    <img :src="listing.image ? `/storage/${listing.image}` : '/storage/images/listings/default.png'" class="w-10 h-10 rounded-full object-cover object-center" alt="">
                                    <h4 class="font-bold">
                                        {{ listing.title }}
                                        <i :class="`fa-solid fa-circle-${listing.status === 'pending' ? 'xmark' : 'check'} text-${listing.status === 'pending' ? 'red' : 'green'}-500`"></i>
                                    </h4>
                                </div>
                            </td>
                            <td class="w-1/4 py-3 pr-3 text-right text-indigo-500">
                                <Link :href="route('listing.show', listing.id)" v-if="listing.approved">
                                    View
                                </Link>
                            </td>
                            <td class="w-1/5 py-3 pr-3 text-right text-indigo-500">
                                <Link :href="route('listing.edit', listing.id)" v-if="listing.approved">
                                    Edit
                                </Link>
                            </td>
            ...
            <div>
                <PaginationLinks :paginator="listing" />
            </div>
        </div>
